'''
    ...
    1. CNN
    2. CNN + KNN
    3. CNN + KNN + translation
    4. CNN + KNN + overlap
    5. CNN + KNN + overlap + translation
'''
...
def load_all_data(raw_train, raw_dev, raw_test, stoplist):
    ...
    train_pmids = raw_train.query_pmids[:1000]
    dev_pmids = raw_dev.query_pmids[:500]
...
if __name__ == '__main__':
    ...
    raw_train = kdata2.Data(os.path.join(my_data_dir, "L5K"), 15000, rm_list)
    raw_train.run()
    rm_list = list(set(raw_test.query_pmids)|set(raw_train.query_pmids))
    print "raw train ready"
    raw_dev = kdata2.Data(os.path.join(my_data_dir, "L1K"), 3000, rm_list)
    raw_dev.run()
    print "raw dev ready"
    # print "====================="
