...
class Eval_BM25_KNN_candidates(object):
    ...
    def __init__(self, data_dir, out_dir, time_span):
        ...
        print "Use index ", self.index_file
        
        self.pmid_base_dir = os.path.join(self.data_dir, "pmid_docs_by_year")
        self.medline_base_dir = os.path.join(self.data_dir, "medline_docs_by_year")
        self.tiab_base_dir = os.path.join(self.data_dir, "tiab_by_year")
        self.mesh_base_dir = os.path.join(self.data_dir, "mesh_by_year")
        startyear, endyear = time_span.split("_")
        self.years = range(int(startyear), int(endyear)+1)
        print "year range: ", self.years
...
if __name__=="__main__":
    ...
    data_dir = '/home/w2wei/data2'
    out_dir = os.path.join(data_dir, 'analysis', 'L1000_query_PMID_on_MEDLINE_%s'%time_span)
    if not os.path.exists(out_dir):
        os.makedirs(out_dir)
    L1000_analysis = Eval_BM25_KNN_candidates(data_dir, out_dir, time_span)
    L1000_analysis.run()
