'''
    Prepare 10K training examples from 1960-2009, prepare the vocabulary, prepare a validation set (1K), prepare test sets (1K). 
    Features: CNN features and KNN feature, no MTI feature
'''
...
def load_all_data(raw_train, raw_dev, raw_test, stoplist):
    ...
    for i in range(len(obj_list)):
        ...
        for pmid in pmidList: 
            ...
            print "cand_pmids num: ", len(cand_pmids)
            cand_pmids = cand_pmids[:20]
            raw_input('check here. load_all_data. remove query pmid from similar pmid list.')            
...
if __name__ == '__main__':
    ...
    train_sample_size = sys.argv[3]
    dev_sample_size = sys.argv[4]
    test_sample_size = sys.argv[5]
    ...
    out_base_dir = "/home/w2wei/exp/"
    outdir = os.path.join(out_base_dir, wkdir)
    lu_data_dir = "/home/w2wei/data/lu_data"
    clean_lu_data_dir = os.path.join(data_dir, "clean")

    ## pre-process the raw data to generate data objects 
    t0=time.time()
    rm_list = [] ## pmids to remove from datasets
    raw_train = cdata.Data(data2_dir, outdir, wkdir, index_dir, time_span, train_sample_size, pmid_year_dict_file, rm_list)
    raw_train.run()
    print len(raw_train.query_pmids)
    print len(raw_train.query_tam)
    print len(raw_train.nbr_dict)
    print len(raw_train.nbr_tam)
    print "raw train ready"
    t1=time.time()
    print "Prepare training data time: ", t1-t0
    print

    t0 = time.time()
    rm_list = raw_train.query_pmids
    raw_dev = cdata.Data(data2_dir, outdir, wkdir, index_dir, time_span, dev_sample_size, pmid_year_dict_file, rm_list)
    raw_dev.run()
    print len(raw_dev.query_pmids)
    print len(raw_dev.query_tam)
    print len(raw_dev.nbr_dict)
    print len(raw_dev.nbr_tam)    
    print "raw dev ready"
    t1 = time.time()
    print "Prepare dev data time: ", t1-t0
    print

    t0=time.time()
    rm_list = list(set(raw_train.query_pmids)|set(raw_dev.query_pmids))
    raw_test = cdata.Data(data2_dir, outdir, wkdir, index_dir, time_span, test_sample_size, pmid_year_dict_file, rm_list)
    raw_test.run()
    print len(raw_test.query_pmids)
    print len(raw_test.query_tam)
    print len(raw_test.nbr_dict)
    print len(raw_test.nbr_tam)  
    print "raw test ready"
    t1=time.time()
    print "Prepare test data time: ", t1-t0
    print
    
    dev_rm_list = list(set(raw_dev.query_pmids)-set(raw_train.query_pmids))
    test_rm_list = list(set(raw_test.query_pmids)-(set(raw_dev.query_pmids)|set(raw_train.query_pmids)))

    print "dev_rm_list: ", len(dev_rm_list)
    print "test_rm_list: ", len(test_rm_list)
