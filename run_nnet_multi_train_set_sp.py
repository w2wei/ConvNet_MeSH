...
def main():
    ...
    cPickle.dump(train_nnet, open(nnet_fname, 'wb'), protocol=cPickle.HIGHEST_PROTOCOL)

    total_params = sum([numpy.prod(param.shape.eval()) for param in params])
    print 'Total params number:', total_params

    ## learn parameters from subsets
    ...
    for subset in os.listdir(subset_dir):
        sub_dir = os.path.join(subset_dir, subset)
        ## Load training subsets
        q_train = numpy.load(os.path.join(sub_dir, 'train.questions.npy'))
        a_train = numpy.load(os.path.join(sub_dir, 'train.answers.npy'))
        q_overlap_train = numpy.load(os.path.join(sub_dir, 'train.q_overlap_indices.npy'))
        a_overlap_train = numpy.load(os.path.join(sub_dir, 'train.a_overlap_indices.npy'))
        q_knn_count_train = numpy.load(os.path.join(sub_dir, 'train.q_knn_counts.npy'))
        a_knn_count_train = numpy.load(os.path.join(sub_dir, 'train.a_knn_counts.npy'))
        q_mti_train = numpy.load(os.path.join(sub_dir, 'train.q_mti.npy'))
        a_mti_train = numpy.load(os.path.join(sub_dir, 'train.a_mti.npy'))
        y_train = numpy.load(os.path.join(sub_dir, 'train.labels.npy'))

        train_nnet = cPickle.load(file(nnet_fname, 'rb'))
        params = train_nnet.params
        # total_params = sum([numpy.prod(param.shape.eval()) for param in params])
        # print 'Total params number:', total_params
        ...
        print    
        fname = os.path.join(subres_dir, '{}_best_dev_params.epoch={:02d};batch={:05d};dev_acc={:.2f}.dat'.format(subset, epoch, i, best_dev_acc))
