'''
    Retrieve all PMID for given years from PubMed, and then download MEDLINE associated with these PMID, arond 10MB every 5K MEDLINE records
...
class GetMEDLINE(object):
    ...
    def get_PMID(self):
        ...
        while newPMID>=step:
            try:
                ...
                if os.path.exists(os.path.join(self.pmidDir, fname)):
                    continue
    ...
    def get_medline(self):
        ...
        for pmidDoc in pmidFiles:
            if os.path.exists(os.path.join(self.medlineDir, pmidDoc)):
                continue
