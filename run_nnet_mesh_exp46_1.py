'''
    Exp 46_1
    Training set: 1K from MEDLINE 1997
...
def main():
    ...
    nnet_outdir = 'exp.out/Exp_46_1;ndim={};batch={};max_norm={};learning_rate={};{}'.format(ndim, batch_size, max_norm, learning_rate, ts)
    ...
    def predict_prob_batch(batch_iterator):
        preds = numpy.hstack([pred_prob_fn(batch_x_q, batch_x_a, batch_x_q_knn_count, batch_x_a_knn_count) for batch_x_q, batch_x_a, batch_x_q_knn_count, batch_x_a_knn_count, _ in batch_iterator])
        return preds[:batch_iterator.n_samples]        

    train_set_iterator = sgd_trainer.MiniBatchIteratorConstantBatchSize(numpy_rng, [q_train, a_train, q_knn_count_train, a_knn_count_train,y_train], batch_size=batch_size, randomize=True)
    dev_set_iterator = sgd_trainer.MiniBatchIteratorConstantBatchSize(numpy_rng, [q_dev, a_dev, q_knn_count_dev, a_knn_count_dev, y_dev], batch_size=batch_size, randomize=False)
    test_set_iterator = sgd_trainer.MiniBatchIteratorConstantBatchSize(numpy_rng, [q_test, a_test, q_knn_count_test, a_knn_count_test, y_test], batch_size=batch_size, randomize=False)
    ...
    while epoch < n_epochs:
        timer = time.time()
        for i, (x_q, x_a, x_q_knn_count, x_a_knn_count, y) in enumerate(tqdm(train_set_iterator), 1):
            train_fn(x_q, x_a, x_q_knn_count, x_a_knn_count, y)
            ...
            if i % 5000 == 0 or i == num_train_batches:
    ...
    print "Exp_46_1 MAP on test set: ", test_acc/100.0
    print    
